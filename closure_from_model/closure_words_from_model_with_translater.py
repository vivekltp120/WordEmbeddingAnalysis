...
def generate():
    ...
    if args.vocab_file:
        ...
        outfile_eval = vo_name + "_closure_words_for_eval_script_" + current_time + ".txt"
...
def distance(W, vocab, ivocab, input_term):
    ...
    for x in a:
        try:
            trans_word = translator.translate(ivocab[x], dest='en')
            closureWord.write("%30s\t\t%f\t\t%25s\n" % (ivocab[x], dist[x],trans_word.text))
            eval_file.write("%s " %(ivocab[x]))
            # print("%30s\t\t%f\t\t\t%s\n" % (ivocab[x], dist[x],trans_word.text))
        except ValueError:
            closureWord.write("%30s\t\t%f\t\t%25s\n" % (ivocab[x], dist[x],"Not Available"))
            print("ValueError")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if input_term == 'EXIT':
            ...
            try:
                word_in_english=translator.translate(input_term, dest='en')
                closureWord.write("Input word                "+ input_term+ " --> "+word_in_english.text+'\n')
                eval_file.write("%s " %(input_term))
                distance(W, vocab, ivocab, input_term)
            except ValueError:
                closureWord.write("Input word                " + input_term + " --> " + "Not Available" + '\n')
                distance(W, vocab, ivocab, input_term)
                print("Value Error")
