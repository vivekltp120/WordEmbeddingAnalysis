...
outfile.write('WEM_1 - '+'_'.join(f1_name.split("_")[0:20])+'\n')
outfile.write('WEM_2 - '+'_'.join(f2_name.split("_")[0:20])+'\n')
outfile.write('WEM_3 - '+'_'.join(f3_name.split("_")[0:10])+'\n')

with open(f1_path) as f1, open(f2_path) as f2,open(f3_path) as f3:
      ...
      for a,b,c in zip(w1,w2,w3):
          if(a[0]==b[0]==c[0]):
              ...
              w2=set(b[1:])
              w3=set(c[1:])
              inter=w1.intersection(w2).intersection(w3)
